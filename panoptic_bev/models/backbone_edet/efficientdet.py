# Original source: https://github.com/zylo117/Yet-Another-EfficientDet-Pytorch
# Modified to increase the feature sizes used in the BiFPN layers
import torch.nn as nn
import torch
import torch.nn.functional as F
from torchvision.ops.boxes import nms as nms_torch

from panoptic_bev.models.backbone_edet.efficientnet import EfficientNet as EffNet
...
class ParameterModule(nn.Module):
    def __init__(self, size):
        ...
        self.parameter = nn.Parameter(torch.ones(size, dtype=torch.float32), requires_grad=True)
        self.relu = nn.ReLU()

    def forward(self):
        return self.relu(self.parameter)
...
#         p7_in_resize = self.p6_upsample(p7_in)
#         p7_in_resize = F.interpolate(p7_in_resize, size=(p6_in.shape[2], p6_in.shape[3]), mode="bilinear", align_corners=True)
#         p6_up = self.conv6_up(self.swish(weight[0] * p6_in + weight[1] * p7_in_resize))
...
class EfficientDet(nn.Module):
    ...
    @staticmethod
    def from_pretrained(model, model_name, load_weights=True, advprop=False, num_classes=1000, in_channels=3, ignore_layers=[]):
        if load_weights:
            load_pretrained_weights(model, model_name, ignore_layers)
        return model
