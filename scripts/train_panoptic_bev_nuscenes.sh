...
../../venv/panoptic_bev/bin/python -m torch.distributed.launch --nproc_per_node=2 --master_addr=$(hostname -I | cut -d ' ' -f 1) --master_port=$(python3 get_port.py) train_panoptic_bev.py \
                                    --run_name=run1 \
